...
// FindMounts applies the requested glob (filepath.Match) to mountNames so
// Detect's "mtp:*RICOH*" glob is actually exercised.
...
func (t *fakeTree) FindMounts(_ context.Context, glob string) ([]string, error) {
	var out []string
	for _, name := range t.mountNames {
		match, err := filepath.Match(glob, name)
		if err != nil {
			return nil, err
		}
		if match {
			out = append(out, filepath.Join(t.root, name))
		}
	}
	return out, nil
}
...
// realMountName is the GVFS MTP mount name the GR IV exposes on Linux.
const realMountName = "mtp:host=RICOH_IMAGING_COMPANY__LTD._RICOH_GR_IV_0024081"
...
func TestDetect(t *testing.T) {
	mounts := &fakeTree{
		...
		mountNames: []string{
			realMountName,
			"mtp:host=Supernote_A6402X014970", // Supernote must not match
			"gphoto2:usb 003,004",             // Fuji mount must not match
		},
	}
	d := &Driver{}
	...
	wantSrc := "/gvfs/" + realMountName
	if len(devs) != 1 || devs[0].Driver != "ricoh" || devs[0].Label != "RICOH GR camera" || devs[0].Source != wantSrc {
...
func TestSyncRoutesFiles(t *testing.T) {
	tree := newFakeTree()
	tree.addDir("/src", "Internal Memory")
	tree.addDir("/src/Internal Memory", "DCIM")
	tree.addDir("/src/Internal Memory/DCIM", "100RICOH")
	shotDir := "/src/Internal Memory/DCIM/100RICOH"
	tree.addFile(shotDir, "R0000001.JPG", []byte("jpg"))
	tree.addFile(shotDir, "R0000028.MOV", []byte("mov"))
	tree.addFile(shotDir, "R0000099.DNG", []byte("raw"))
	...
	if err := d.Sync(context.Background(), driver.Device{Driver: "ricoh", Source: "/src"}, env); err != nil {
		t.Fatalf("Sync: %v", err)
	}
...
func TestSyncSkipExisting(t *testing.T) {
	tree := newFakeTree()
	tree.addFile("/src", "R0000099.DNG", []byte("raw"))
	...
	_ = env.Local.WriteFile(context.Background(), filepath.Join(cfg.RicohRAWDest, "R0000099.DNG"), []byte("xxx"), 0o644)
...
func TestSyncSkipsGeotaggedFileNextRun(t *testing.T) {
	tree := newFakeTree()
	tree.addFile("/src", "R0000001.JPG", []byte("jpg")) // source size 3
	...
	jpg := filepath.Join(cfg.RicohJPEGDest(), "R0000001.JPG")
...
func TestSyncRecopiesChangedCameraFile(t *testing.T) {
	tree := newFakeTree()
	tree.addFile("/src", "R0000001.JPG", []byte("jpg")) // size 3
	...
	tree2 := newFakeTree()
	tree2.addFile("/src", "R0000001.JPG", []byte("jpg-now-much-larger")) // size 19
	...
	got, _ := env.Local.ReadFile(context.Background(), filepath.Join(cfg.RicohJPEGDest(), "R0000001.JPG"))
...
func TestSyncGeotagFailureRollsBack(t *testing.T) {
	tree := newFakeTree()
	tree.addFile("/src", "R0000001.JPG", []byte("jpg"))
